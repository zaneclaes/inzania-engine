...
//
//  2. AGENT HOOKS — renders `ci/agent-hooks.json` into Claude's `.claude/settings.json`, Codex's
//     `.codex/hooks.json` and Grok's `.grok/hooks/agent-guards.json`. The manifest is the one definition of
//     each guard; every runtime calls it through the one adapter, `ci/RunAgentHook.cs`. Only the entries that
//     run that adapter are rewritten — hooks a plugin or the operator added to the same file are kept — and
//     `--check` reports drift in any of the three.
//
...
bool InstallAgentHooks(out List<string> scripts) {
  ...
  const string claudeRoot = "$CLAUDE_PROJECT_DIR/";
  const string codexRoot = "$(git rev-parse --show-toplevel)/";
  // Claude and Grok both expose the project root as $CLAUDE_PROJECT_DIR; Codex has no such variable.
  return RenderAgentHooks("Claude", Path.Combine(root, ".claude", "settings.json"), entries, claudeRoot + adapter,
           (script, arguments) => HookCommand(claudeRoot, adapter, script, arguments), statusMessages: true, description: null)
      && RenderAgentHooks("Codex", Path.Combine(root, ".codex", "hooks.json"), entries, codexRoot + adapter,
           (script, arguments) => HookCommand(codexRoot, adapter, script, arguments), statusMessages: false,
           description: "Entries that run ci/RunAgentHook.cs are generated by ci/install-hooks.sh from the engine and repo agent-hook manifests; edit the manifests, not them. Other entries are preserved.")
      && RenderAgentHooks("Grok", Path.Combine(root, ".grok", "hooks", "agent-guards.json"), entries, claudeRoot + adapter,
           (script, arguments) => HookCommand(claudeRoot, adapter, script, arguments), statusMessages: false, description: null);
}

// One renderer for every agent runtime. All three read the same hook document shape
// ({ hooks: { <event>: [ { matcher, hooks: [ { type, command, timeout } ] } ] } }), and a runtime's file may also hold
// hooks another tool installed (a plugin's, the operator's), so this never rewrites the file wholesale: it removes only
// the entries whose command runs this installer's adapter, re-adds every manifest entry, and keeps every other key and
// entry as it was. Rendering both manifests together is essential: a product hook may be a shell script outside
// .agents/hooks, and processing the manifests independently would otherwise re-add it forever. The adapter path is the
// ownership marker, so an unmanaged hook must not invoke it directly.
bool RenderAgentHooks(string agent, string path, IEnumerable<(ManifestHook Hook, string Prefix)> entries, string ownerMarker,
                      Func<string, IEnumerable<string>?, string> command, bool statusMessages, string? description) {
  string relative = Path.GetRelativePath(root, path).Replace('\\', '/');
  string before = File.Exists(path) ? File.ReadAllText(path) : "";
  Dictionary<string, object?> document;
  try {
    document = before.Trim().Length > 0
      ? ZJson.DeserializeObject<Dictionary<string, object?>>(null, before, Lenient()) ?? new Dictionary<string, object?>()
      : new Dictionary<string, object?>();
  } catch (Exception e) {
    Console.Error.WriteLine($"[install] {relative} is not valid JSON, left alone: {e.Message}");
    return false;
  }

  if (description != null) document["description"] = description;
  if (document.GetValueOrDefault("hooks") is not Dictionary<string, object?> events) document["hooks"] = events = new Dictionary<string, object?>();

  // Where each event and each matcher group sat before stripping. An event or group holding only generated entries is
  // removed and then re-created, and without this it would come back at the end, so every run would reorder the file
  // and never converge.
  var eventOrder = events.Keys.ToList();
  var groupOrder = events.ToDictionary(e => e.Key, e => (e.Value as List<object?> ?? []).OfType<Dictionary<string, object?>>()
    .Select(g => g.GetValueOrDefault("matcher") as string ?? "").ToList());
  ...
  foreach (string evt in events.Keys.ToList()) {
    ...
    for (int g = groups.Count - 1; g >= 0; g--) {
      ...
      hooks.RemoveAll(e => Command(e).Contains(ownerMarker, StringComparison.Ordinal));
  ...
  foreach (var (entry, prefix) in entries) {
    ...
    var node = new Dictionary<string, object?> { ["type"] = "command", ["command"] = command(prefix + entry.Script!, entry.Arguments) };
    if (entry.Timeout is int t) node["timeout"] = t;
    if (statusMessages && !string.IsNullOrWhiteSpace(entry.StatusMessage)) node["statusMessage"] = entry.StatusMessage;
    hooks.Add(node);
  }

  // Canonical ordering of the generated entries makes repeated remove-and-readd passes converge. Entries this installer
  // does not own keep their relative order and stay ahead of the generated ones in a shared group.
  foreach (object? value in events.Values) {
    if (value is not List<object?> groups) continue;
    foreach (object? group in groups) {
      if (group is not Dictionary<string, object?> d || d.GetValueOrDefault("hooks") is not List<object?> hooks) continue;
      var foreign = hooks.Where(h => !Command(h).Contains(ownerMarker, StringComparison.Ordinal)).ToList();
      var owned = hooks.Where(h => Command(h).Contains(ownerMarker, StringComparison.Ordinal))
                       .OrderBy(Command, StringComparer.Ordinal).ToList();
      hooks.Clear();
      hooks.AddRange(foreign);
      hooks.AddRange(owned);
    }
  }

  // Restore the original event and group positions; anything new goes after them in ordinal order.
  foreach (var (evt, value) in events) {
    if (value is not List<object?> groups) continue;
    var known = groupOrder.GetValueOrDefault(evt) ?? [];
    var ordered = groups.OrderBy(g => {
      string matcher = (g as Dictionary<string, object?>)?.GetValueOrDefault("matcher") as string ?? "";
      int at = known.IndexOf(matcher);
      return at >= 0 ? at : int.MaxValue;
    }).ThenBy(g => (g as Dictionary<string, object?>)?.GetValueOrDefault("matcher") as string ?? "", StringComparer.Ordinal).ToList();
    groups.Clear();
    groups.AddRange(ordered);
  }
  var reordered = new Dictionary<string, object?>();
  foreach (string evt in eventOrder.Where(events.ContainsKey)) reordered[evt] = events[evt];
  foreach (string evt in events.Keys.Where(k => !reordered.ContainsKey(k)).OrderBy(k => k, StringComparer.Ordinal)) reordered[evt] = events[evt];
  document["hooks"] = reordered;

  string after = Write(document) + "\n";
  if (Normalize(before) == Normalize(after)) {
    Console.WriteLine($"[install] {agent} hooks current.");
    return true;
  }

  changes.Add($"{relative} (agent hooks)");
  if (check) return true;
  Directory.CreateDirectory(Path.GetDirectoryName(path)!);
  File.WriteAllText(path, after);
  Console.WriteLine($"[install] {agent} hooks written to {relative}.");
  return true;
}
...
static string HookCommand(string projectRoot, string adapter, string guard, IEnumerable<string>? arguments) =>
  "dotnet run \"" + projectRoot + adapter + "\" -- --guard \"" + projectRoot + guard + "\"" + Arguments(arguments);
