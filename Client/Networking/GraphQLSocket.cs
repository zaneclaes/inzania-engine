...
public class GraphQlSocket<TData> : TransientObject, IActivate where TData : class {
  ...
  public GraphQlSocket(
    ITuneContext context, string subscriptionUrl, GraphRequest req, Dictionary<string, string> headers) : base(context) {
  ...
  public DateTime LastHeartbeat { get; private set; } = IZEnv.Now;
...
#pragma warning restore 67
  ...
  private void HandleMessage(byte[] bytes) {
    ...
    Dictionary<string, object>? obj = TuneJson.DeserializeObject<Dictionary<string, object>>(Context, message);
    ...
    if (subType.Equals("connection_ack")) {
      ...
      try {
        object? payload = obj["payload"] ?? throw new RemoteTuneException(Context, "No payload");
        ...
        else throw new InternalTuneException(Context, "No data object returned");
      } catch (Exception e) {
