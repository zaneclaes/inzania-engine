...
public class TuneGoogleAnalytics : LogicBase, ITuneAnalytics {
  ...
  public static AnalyticsStream GetStreamForContext(ITuneContext context) =>
    context.App.Env <= IZEnvironment.Staging ? StagingStream : ProdStream;
  ...
  public async Task Configure(IAnalyticsSink sink, ITuneIdentity? identity = null) {
    if (identity == null) {
      if (_visitor == null) {
    ...
    await ((ITuneAnalytics) this).SetIdentity(identity);
    ProcessQueue();
  }

  public TuneGoogleAnalytics(ITuneContext context) : base(context) { }
  ...
  public Task PageView(string path, string? title = null) {
    ...
    return ((ITuneAnalytics) this).SendEvent("page_view", new PageViewEventParams {
      Path = path,
      Title = title
    }); // data
  }
  ...
  public Task ScreenView(string name, string? klass = null) =>
    ((ITuneAnalytics) this).SendEvent("screen_view", new ScreenViewEventParams {
      Name = name,
      Class = klass
    }); // data

  public Task Share(string method) =>
    ((ITuneAnalytics) this).SendEvent("share", new MethodEventParams {
      Method = method
    });

  public Task LoginBegin(string method) =>
    ((ITuneAnalytics) this).SendEvent("login_begin", new MethodEventParams {
      Method = method
    });

  public Task LoginEnd(string method) =>
    ((ITuneAnalytics) this).SendEvent("login", new MethodEventParams {
      Method = method
    });

  public Task SignUp(string method) =>
    ((ITuneAnalytics) this).SendEvent("sign_up", new MethodEventParams {
      Method = method
    });

  public Task Search(string searchTerm) =>
    ((ITuneAnalytics) this).SendEvent("search", new SearchEventParams {
      SearchTerm = searchTerm
    });

  public Task EarnPoints(long score, int? level = null, string? character = null) =>
    ((ITuneAnalytics) this).SendEvent("post_score", new ScoreEventParams {
      Score = score,
      Level = level,
      Character = character
    });
  ...
  public Task SelectContent(string contentType, string contentId) =>
    ((ITuneAnalytics) this).SendEvent("select_content", new ContentEventParams {
      ContentType = contentType,
      ContentId = contentId
    });

  public Task Exception(string desc, bool fatal = false) =>
    ((ITuneAnalytics) this).SendEvent("exception", new ExceptionEventParams {
      Description = desc,
      IsFatal = fatal
    });
