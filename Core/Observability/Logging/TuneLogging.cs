...
public static class TuneLogging {
  ...
  private static object TransformLoggableObject(object obj, int level = 0) {
    var t = obj.GetType();
    var desc = TuneTypeDescriptor.FromType(t);
    ...
    //       IZEnv.Log.Information("[LIST] NO  {n} {t}", t2.Name, listType.Name);
    //   }
    //   ret.Add(p.Name, val);
    // }
    return level > 0 ? ret : TuneJson.SerializeObject(ret, new TuneJsonSerializationOpts {
      PrettyPrint = true
    });
  }
  ...
  public static object TransformObject<TObj>(object mXml) {
    try {
      ...
      IZEnv.Log.Error(e, "[LOG] failed to transform {type} from {stack}", mXml.GetType().Name, new TuneTrace(e.StackTrace));
      return mXml;
    }
  }
  ...
  public static TBuilder WithTuneData<TBuilder>(this TBuilder c) where TBuilder : LogBuilder {
    c
      ...
      .TransformObject<Exception>((ex) => TuneError.Guard(ex))
