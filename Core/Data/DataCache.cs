...
public abstract class DataCache : LogicBase {
  ...
  public DataCache(ITuneDataRepository repo) : base(repo.Context) {
    Repository = repo;
  }
...
public class DataCache<T> : DataCache, ITuneQueryable<T> {
  private readonly List<T> _items = new List<T>();

  public DataCache(ITuneDataRepository repo) : base(repo) { }
  ...
  public override void Cache(params object[] objects) {
    foreach (object? o in objects) {
      if (!(o is T obj)) throw new InternalTuneException(Context, $"Object {o.GetType().Name} is not a {typeof(T).Name}");
      _items.Add(obj);
    }
  }
