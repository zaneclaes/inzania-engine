...
public static class DataModelLoader {
  private static ITuneDataRepository GetRepository(this IQueryable q, ITuneContext? context = null) {
    if (q is ITuneQueryable tq) return tq.Repository;
  ...
  private static ITuneContext GetContext(this IQueryable q, ITuneContext? context = null) {
    if (q is ITuneQueryable tq) return tq.Context;
    if (context == null) {
      context = IZEnv.SpawnRootContext();
      context.Log.Warning("[DATA] spawning default context for {q} ({p}); stacktrace: {stacktrace}",
        q.GetType().GenericTypeArguments.First().Name, q.Expression.GetType(), new TuneTrace(new StackTrace().ToString()).ToString());
    }
    return context;
  }

  // private static async Task<List<T>> LoadListAsync<T>(
  //   this IQueryable<T> queryable, ITuneContext? context = null, bool executeLocked = true
  // ) {
  ...
  public static async Task<Dictionary<TK, T>> LoadDataModelDictionary<TK, T>(
    this IQueryable<T> queryable, Func<T, TK> getKey, ITuneContext? context = null
  ...
  public static async Task<List<T>> LoadDataModelsAsync<T>(
    this IQueryable<T> queryable, ITuneContext? context = null) where T : DataObject {
    context = queryable.GetContext(context);
  ...
  public static async Task<ILookup<TKey, TData>> LoadLookupAsync<TKey, TData>(
    this IQueryable<TData> queryable, Func<TData, TKey> lookup, ITuneContext? context = null
  ...
  public static ITuneQueryable<TData> FilterKeyIn<TData, TKey>(
    this ITuneQueryable<TData> queryable, string key, params TKey[] vals
  ) where TData : DataObject {
  ...
  public static Task<T?> LoadScalarAsync<T>(this IQueryable<T> q, ITuneContext? context = null) {
    context ??= q.GetContext(context);
    return q.GetRepository(context).ExecuteFirstOrDefaultAsync(context, q);
  }

  public static Task<List<T>> LoadScalarsAsync<T>(this IQueryable<T> q, ITuneContext? context = null) {
    context ??= q.GetContext(context);
    return q.GetRepository(context).ExecuteListAsync(context, q);
  }

  public static async Task<long> LoadSumAsync<T>(
    this IQueryable<T> queryable, Expression<Func<T, long>> func, ITuneContext? context = null
  ) {
    context = queryable.GetContext(context);
  ...
  public static async Task<long> LoadCountAsync<T>(
    this IQueryable<T> queryable, ITuneContext? context = null
  ) {
    context = queryable.GetContext(context);
  ...
  public static async Task<T> LoadDataModelAsync<T>(
    this IQueryable<T> queryable, string missingException, ITuneContext? context = null
  ) where T : DataObject {
    context = queryable.GetContext(context);
  ...
  //   this ITuneContext context, string id, Func<TModel, DataState, Task>? creator = null
  ...
  public static ITuneQueryable<TModel> QueryForModelIds<TModel, TKey>(
    this ITuneContext context, params TKey[] id
  ...
  public static ITuneQueryable<TModel> AsTuneQueryable<TModel>(this IQueryable<TModel> q, ITuneQueryProvider provider) where TModel : DataObject {
    if (q is ITuneQueryable<TModel> tq) return tq;
    provider.Log.Warning("[QUERY] converting {type} to TuneQueryable<{dm}> via {other}", q.GetType(), typeof(TModel), provider.GetType());
    return new TuneQueryable<TModel>(provider, q);
  }

  public static ITuneQueryable<TSource> Filter<TSource>(this ITuneQueryable<TSource> q, Expression<Func<TSource, bool>> predicate)
    where TSource : DataObject => q.Where(predicate).AsTuneQueryable(q.QueryProvider);

  public static Task<TModel> Upsert<TModel>(this ITuneQueryable<TModel> q, Action<TModel> creator) where TModel : DataObject, new() =>
    q.QueryProvider.Context.Upsert(q, creator);

  public static Task<TModel> Upsert<TModel>(
    this ITuneContext context, IQueryable<TModel> query, Action<TModel> creator
  ...
  public static Task<TModel> UpsertKey<TModel, TKey>(
    this ITuneContext context, TKey id, Action<TModel>? creator = null
  ...
  public static Task<TModel> UpsertId<TModel>(
    this ITuneContext context, string id, Action<TModel>? creator = null
  ) where TModel : ModelKey<string>, new() => context.UpsertKey(id, creator);

  public static async Task<TModel> UpsertModel<TModel>(
    this ITuneContext context, IQueryable<TModel> query, Func<TModel, DataState, Task>? creator = null
