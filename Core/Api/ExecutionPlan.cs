...
public class ExecutionPlan {
  ...
  public TuneTypeDescriptor ReturnType { get; }
  ...
  private readonly TuneMethodDescriptor _method;

  private static readonly Dictionary<string, ExecutionPlan> Plans = new Dictionary<string, ExecutionPlan>();

  public static ExecutionPlan Load(ITuneContext context, Type parent, string operationName, ResultSet resultSet) =>
  ...
  public static ExecutionPlan Load(ITuneContext context, ApiExecutionType op, string operationName, ResultSet resultSet) =>
  ...
  public Dictionary<string, Tuple<TuneTypeDescriptor, object?>> CoerceArgs(List<object?> args) {
    Dictionary<string, Tuple<TuneTypeDescriptor, object?>> ret = new Dictionary<string, Tuple<TuneTypeDescriptor, object?>>();
    ...
    for (int i = 0; i < _method.Parameters.Count; i++) {
      var tuneType = TuneTypeDescriptor.FromType(_method.Parameters[i].ParameterType);
      ...
      if (_method.Parameters[i].ParameterType == typeof(IFileUpload) && args[i] is IFileUpload upload) {
        // IZEnv.Log.Information("COERCE {arg}",args[i]?.GetType());
        argVal = upload;
      }

      ret[_method.Parameters[i].FieldName] = new Tuple<TuneTypeDescriptor, object?>(tuneType, argVal);
    }
  ...
  private static JsonNode? PrepareArg(object? arg) {
    ...
    var desc = TuneTypeDescriptor.FromType(arg.GetType());
  ...
  public string ToGraphQLDocument() {
    ...
    // IZEnv.Log.Information("[OP] {query}", op);
