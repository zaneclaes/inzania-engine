...
/// conscious decision, not an accident. The edit-time counterpart is the `ApiAuthGuard` hook,
/// which additionally sees method BODIES (identity dereferences, data writes) that reflection
/// cannot. Reflection only sees *loaded* assemblies, so the test must touch a type from each API
/// project first (constructing any one API class is enough).
/// </summary>
