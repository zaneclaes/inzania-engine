...
public class TuneQueryAccessor : IOperationDocumentStorage {
  public TuneQueryAccessor(IFragmentProvider frag) { _provider = frag; }
  ...
  private OperationDocument GenerateQuery(string queryId) {
    IZEnv.Log.Information("[QUERY] ID {id}", queryId);
    ...
    if (parts.Count > 1) IZEnv.Log.Warning("[QUERY] {id} has unused parts", queryId);
    string fieldName = parts.First().ToFieldName();
    IZEnv.Log.Debug("[QUERY] {type} {fieldName} {format}", executionType, fieldName, format);
    ...
    IZEnv.Log.Debug("[QUERY] {id} => {q}", queryId, query);
