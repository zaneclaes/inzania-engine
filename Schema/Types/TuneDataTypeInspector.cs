...
public class TuneDataTypeInspector : DefaultTypeInspector, ITypeInspector {
  public override bool IsMemberIgnored(MemberInfo member) {
    IZEnv.Log.Verbose("[MEMBER] {type} {t2}", member.DeclaringType, member.ReflectedType);
  ...
  //     et.Ob
  //     IZEnv.Log.Information("[ET] {name} = {@auth}", member.Name, et);
  //   }
  //   return et;
  // }
  ...
  public new void ApplyAttributes(
    ...
    if (descriptor is ObjectFieldDescriptor ofd &&
      ...
      // IZEnv.Log.Information("[ATTR] {ctxt} {@desc}", descriptor.GetType().Name, attr);
