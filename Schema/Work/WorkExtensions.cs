...
public static class WorkExtensions {
  public static void LogErrors(this IOperationResult result, ITuneLogger log, HttpStatusCode code = HttpStatusCode.OK) {

    List<IError> errors = result.Errors?.ToList() ?? new List<IError>();
    log.LogErrors(errors, code);

  }
  public static void LogErrors(this ITuneLogger log, List<IError> errors, HttpStatusCode code = HttpStatusCode.OK) {
    // List<IError> errors = result.Errors?.ToList() ?? new List<IError>();
    if (errors.Any()) {
      ...
      // }
      var logLevel = code == HttpStatusCode.OK ? TuneEventLevel.Debug : TuneEventLevel.Error;
  ...
  public static Task<IOperationResult> ExecuteInternalApiCall(this OperationRequestBuilder req, ITuneContext context, ClaimsPrincipal claimsPrincipal) =>
    ExecuteInternalApiCall(req, context, new UserState(claimsPrincipal));

  public static async Task<IOperationResult> ExecuteInternalApiCall(this OperationRequestBuilder req, ITuneContext context, UserState userState) {
