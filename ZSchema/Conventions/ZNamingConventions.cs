...
public class ZNamingConventions : DefaultNamingConventions {
  ...
  //   //     str = str.Substring(1);
  //   //   }
  //   //   ZEnv.Log.Information("[VAL] {temp} {str}", pre, str);
  //   //   return pre + str;
  //   // }
  ...
  public override string? GetMemberDescription(
    MemberInfo member,
    MemberKind kind) {
    // ZEnv.Log.Information("[MEMBER DESC] {name}", member.Name);
  ...
  public override string? GetArgumentDescription(ParameterInfo parameter) {
    // ZEnv.Log.Information("[ARG DESC] {name}", parameter.Name);
...
public static class ZNamingExtensions {
