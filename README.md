...
Follow the root `AGENTS.md` **Safe change protocol**. `ZCore`, `ZClient`, `ZExt`, `ZP2P`, `ZSerilog` are symlinked into Unity: after editing them run `dotnet build Chordzy.sln -c Release --no-incremental -warnaserror -tl:off`, `dotnet test TuneTests/TuneTests.csproj -c Release --no-build`, then have the rollout orchestrator attach the persistent Hub-opened Editor for its compile; the separate CI player jobs retain batchmode. C# 10 / netstandard2.0 in those five projects, `ZTask` not `Task`, `Z_UNITY` gates. `ZData`/`ZSchema`/`ZServer`/`ZDataDog`/`ZTests` are .NET-only (build + tests suffice). Remember this is a submodule: commit here first, then bump the pointer in the parent repo.
