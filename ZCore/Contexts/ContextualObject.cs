...
public abstract class ContextualObject : IDisposable, IEventEnricher {
  ...
  public void ProvideContext(IZContext context) {
    if (_context == null) _context = context;
  }

  // Use with care... bypasses checks to ensure context on this and all child objects
  public bool EnforceContext(IZContext context) {
  ...
  private void EnforceDataContext(IZContext context, HashSet<string> breadcrumbs) {
    _context = context;
    ...
    List<ZPropertyDescriptor>? props = ApiType.ObjectDescriptor.ObjectProperties.Values.ToList(); //.Where(p => p.ExecutionMethod != null);
